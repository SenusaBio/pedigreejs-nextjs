...
function addButtons(options) {
	...
	for(let i=0; i<btns.length; i++) {
		lis += '<span style="display: inline-block; width: 24px; height: 24px; text-align: center; line-height: 24px;">';
		lis += '<i class="fa ' + btns[i].fa + '" aria-hidden="true" title="'+ btns[i].title + '"' +
...
function reset(opts) {
	...
	// Clear disease configuration from localStorage
	try {
		localStorage.removeItem('pedigreejs_diseases');
		opts.diseases = [];
		// Clear disease list UI
		$('#disease-list').empty();
		// Trigger custom event to notify UI components about disease reset
		$(document).trigger('diseasesReset', [opts]);
	} catch (e) {
		console.warn('Could not clear diseases from localStorage');
	}
...
function get_printable_svg(opts) {
	...
	let a4 = {w: (800-40), h: (1100-50)}; // A4 lebih lebar
	...
	let scaled_width = d.w * k;
	let center_x = (a4.w - scaled_width) / 2;
	let top_margin = 0; // Small margin from top
	
	let xi = -(b.xmin-(opts.symbol_size))*k + center_x;
	let yi = -(b.ymin-(opts.symbol_size))*k + top_margin;

	svg.attr('width', a4.w);
	svg.attr('height', a4.h);
	
	// Remove background and border for print
...
function print(el, id){
	...
	headContent += "<style>@page { margin: 0.5in; @top-left { content: 'Pedigree Chart - " + new Date().toLocaleDateString() + "'; font-size: 12px; } @bottom-center { content: 'Page ' counter(page) ' - Generated: " + new Date().toLocaleString() + "'; font-size: 10px; } } body { margin: 0; padding: 0; background: white; } svg { display: block; margin: auto; background: none !important; } rect { fill: none !important; stroke: none !important; }</style>";
...
function load_data(d, opts) {
	...
	try{
		...
		opts.auto_scale_on_load = true;	// flag for auto scale
		$(document).trigger('rebuild', [opts]);
...
function getPartnerIdx(dataset, anode) {
	for(let i=0; i<dataset.length; i++) {
		...
		if(anode.name === bnode.mother)
			return getIdxByName(dataset, bnode.father);
		else if(anode.name === bnode.father)
			return getIdxByName(dataset, bnode.mother);
	}
	return -1;
}
...
function addWidgets(opts, node) {
	...
	let widgets = {
		...
		'addsibling': {
			'text': '\uf234', 
			'title': 'add sibling', 
			'fx': fx, //(opts.symbol_size) - 40
			'fy': fy},
		'addpartner': {
			'text': '\uf0c1', 
			'title': 'add partner', 
			'fx': (opts.symbol_size/2) - 2.5, 
			'fy': fy//- opts.symbol_size + 10.5
		},
		'addparents': {
...
function addchild(dataset, node, sex, nchild, twin_type) {
	...
	for (let i = 0; i < nchild; i++) {
		let child = {"name": makeid(4), "sex": sex,
...
function addparents(opts, dataset, name) {
	...
	let i;
	if(depth === 1) {
		mother = {"name": makeid(4), "sex": "F", "top_level": true};
		father = {"name": makeid(4), "sex": "M", "top_level": true};
		...
		if( (!add_lhs && node_father.data.id > node_mother.data.id) ||
			...
			midx = getIdxByName(dataset, node.father);
		else
			midx = getIdxByName(dataset, node.mother);
		...
		for(i=0; i<node.parent_node.length; i++){
			...
			for(j=0; j<children.length; j++) {
				...
				if(child){
					...
					let ptr;
					if(ptrs.length > 0)
						ptr = getNodeByName(dataset, ptrs[0]);
...
function addLabels(opts, node) {
	...
	addLabel(opts, node, -(0.4 * opts.symbol_size), -(0.1 * opts.symbol_size),
			function(d) {
				...
				return '' in d.data ? d.data.display_name : '';}, undefined, ['display_name']); // hide display_name
	...
	for(let i=0;i<opts.diseases.length; i++) {
		...
		addLabel(opts, node, -opts.symbol_size,
				...
				function(d) {
					...
					let dis = disease.replace(/_/g, ' ');
					console.log('After underscore replace:', dis);
					if(dis.includes('cancer')) {
						dis = dis.replace('cancer', 'ca.');
						console.log('After cancer replace:', dis);
					}
					// Check for diagnosis age
					if(disease+'_diagnosis_age' in d.data) {
						return dis +": "+ d.data[disease+'_diagnosis_age'];
					}
					// Check for boolean disease status
					else if(disease in d.data && d.data[disease] === true) {
						console.log('Final display:', dis + ": +");
						return dis + ": +";
					}
					return '';
	...
	for(let l=0; l<arr.length; l++) {
		...
		if(d.data[this_label]) {
			if(this_label === 'alleles') {
				...
				txt += "SB";
			} else if(this_label.match("_diagnosis_age$")) {
				// Handle diagnosis age labels
				let disease_name = this_label.replace('_diagnosis_age', '').replace(/_/g, ' ');
				if(disease_name.indexOf('cancer') !== -1) {
					disease_name = disease_name.replace('cancer', 'ca.');
				}
				txt += disease_name + ": " + d.data[this_label];
				...
				txt += (r === 'P' ? '+ ' : (r === 'N' ? '- ' : ' '));
			} else if(typeof d.data[this_label] === 'boolean') {
				// Handle boolean disease status
				if(d.data[this_label] === true) {
					let disease_name = this_label.replace(/_/g, ' ');
					if(disease_name.indexOf('cancer') !== -1) {
						disease_name = disease_name.replace('cancer', 'ca.');
					}
					txt += disease_name + ": +";
...
function node_has_label(d, labels) {
	for(let l=0; l<labels.length; l++) {
		...
		// Check for diagnosis age labels
		if(labels[l] + '_diagnosis_age' in d.data) return true;
		// Check for boolean disease status
		if(labels[l] in d.data && typeof d.data[labels[l]] === 'boolean' && d.data[labels[l]] === true) return true;
	}
	return false;
}
...
function build(options) {
	let opts = $.extend({ // defaults
		...
		dataset: [ {"name": "m21", "display_name": "father", "sex": "M", "top_level": true},
				   {"name": "f21", "display_name": "mother", "sex": "F", "top_level": true},
				   {"name": "ch1", "display_name": "child", "sex": "F", "mother": "f21", "father": "m21", "proband": true}],
		width: 600,
		height: 400,
		symbol_size: 35,
	...
	// Pre-calculate scale if auto scale on load is enabled
	if(opts.auto_scale_on_load) {
		try {
			// Get rough dimensions for scale calculation
			let temp_tree_dimensions = get_tree_dimensions(opts);
			let svg_dimensions = get_svg_dimensions(opts);
			
			// Calculate optimal scale
			let scale_x = svg_dimensions.width / temp_tree_dimensions.width;
			let scale_y = svg_dimensions.height / temp_tree_dimensions.height;
			let optimal_scale = Math.min(scale_x, scale_y, 1) * 0.9; // 90% to add padding
			
			// Apply pre-calculated scale to zoom
			if(optimal_scale < 1) {
				opts.pre_scale = optimal_scale;
			}
		} catch(e) {
			// Ignore errors in pre-calculation
		}
	}
 ...
 */
	...
	partners = ped.selectAll(".partner")
		...
		.enter()
			...
			.attr('d', function(d, _i) {
				...
				if(consanguity$1) {  // consanguinous, draw double line between partners
	...
	// Apply pre-calculated scale if available
	if(opts.pre_scale && opts.auto_scale_on_load) {
		setTimeout(function(){ 
			let svg = d3.select("#"+opts.targetDiv).select("svg");
			let zm = d3.zoom();
			svg.call(zm.scaleTo, opts.pre_scale);
			scale_to_fit(opts); // Fine-tune positioning
			opts.auto_scale_on_load = false;
			delete opts.pre_scale;
		}, 50);
	} else if(opts.auto_scale_on_load) {
		setTimeout(function(){ 
			scale_to_fit(opts); 
			opts.auto_scale_on_load = false;
		}, 100);
...
function group_top_level(dataset) {
	...
	for(let i=0;i<dataset.length;i++) {
		if(getDepth(dataset, dataset[i].name) === 2)
...
export { pedigree as pedigreejs, extras as pedigreejs__extras, canrisk_file as pedigreejs_canrisk_file, popup_form as pedigreejs_form, io as pedigreejs_io, pedcache as pedigreejs_pedcache, utils as pedigreejs_utils, widgets as pedigreejs_widgets, zoom as pedigreejs_zooming };
