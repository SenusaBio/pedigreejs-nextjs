...
function get_printable_svg(opts) {
	...
	let a4 = {w: (800-40), h: (1165-50)}; // A4 lebih lebar
	...
	let scaled_height = d.h * k;
	let center_x = (a4.w - scaled_width) / 2;
	let center_y = (a4.h - scaled_height) / 2;
	
	let xi = -(b.xmin-(opts.symbol_size))*k + center_x;
	let yi = -(b.ymin-(opts.symbol_size))*k + center_y;
...
function print(el, id){
	...
	headContent += "<style>@page { margin: 0.5in; } body { margin: 0; padding: 0; background: white; } svg { display: block; margin: auto; background: none !important; } rect { fill: none !important; stroke: none !important; }</style>";
