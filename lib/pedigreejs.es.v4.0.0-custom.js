...
// Auto-detect diseases from _diagnosis_age fields and assign gray colors
function auto_detect_diseases(opts) {
	// Clear existing diseases when loading new data
	opts.diseases = [];
	
	let detected_diseases = new Set();
	let existing_diseases = new Set();
	
	// Scan all individuals for disease fields
	$.each(opts.dataset, function(_idx, person) {
		for(let field in person) {
			if(field.endsWith('_diagnosis_age') && person[field] && person[field] !== '0') {
				let disease_type = field.replace('_diagnosis_age', '');
				if(!existing_diseases.has(disease_type)) {
					detected_diseases.add(disease_type);
				}
			}
		}
	});
	
	// Add new diseases with gray colors (preserve existing diseases)
	let gray_colors = ['#CCCCCC', '#B8B8B8', '#A4A4A4', '#909090', '#7C7C7C', '#686868', '#545454', '#404040'];
	let color_index = 0;
	
	Array.from(detected_diseases).forEach((disease) => {
		opts.diseases.push({
			'type': disease,
			'colour': gray_colors[color_index % gray_colors.length]
		});
		color_index++;
	});
	
	// Convert _diagnosis_age fields to disease flags and apply intensity-based coloring
	$.each(opts.dataset, function(_idx, person) {
		let disease_count = 0;
		
		// Count diseases and set flags
		for(let field in person) {
			if(field.endsWith('_diagnosis_age') && person[field] && person[field] !== '0') {
				let disease_type = field.replace('_diagnosis_age', '');
				person[disease_type] = person[field];
				disease_count++;
			}
		}
...
function load_data(d, opts) {
	...
	try {
		...
		// Auto-detect diseases and assign gray colors
		auto_detect_diseases(opts);
		// Trigger disease list update
		$(document).trigger('diseasesUpdated', [opts]);
	} catch(err1) {
		...
		.enter().append("path")
			...
			.attr("fill", function(d, i) {
				...
				if(d.data.ncancers === 0) {
					// Use intensity-based background for affected individuals
					if(d.data.affected === 1) return '#CCCCCC';
					if(d.data.affected === 2) return '#999999';
					if(d.data.affected >= 3) return '#666666';
