...
function init_cache(opts) {
	...
	if (has_browser_storage(opts)) {   // local storage
		try {
			...
			console.warn('Failed to cache dataset due to circular reference:', e);
			return;
		}
...
function showDialog(title, msg, onConfirm, opts, dataset) {
	...
	if(modalBodyInput) modalBodyInput.textContent = msg;
	$("#errModal").modal("show");
}
...
function getPx(opts){
	...
	// Check if element exists and jQuery is available
	if(typeof $ !== 'undefined' && $('#'+opts.targetDiv).length > 0) {
		try {
			return (parseFloat(getComputedStyle($('#'+opts.targetDiv).get(0)).fontSize)*emVal)-1.0;
		} catch(e) {
			// Fallback if getComputedStyle fails
			return emVal * 16 - 1.0; // assume 16px base font size
		}
	}
	// Fallback when jQuery not available or element not found
	return emVal * 16 - 1.0; // assume 16px base font size
}
